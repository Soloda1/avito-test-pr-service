...
- Решение: в актуальных миграциях схема хранит `users.id`, `prs.id`, `prs.author_id`, `pr_reviewers.reviewer_id`, `pr_reviewers.pr_id` в типе `TEXT`. В коде везде перешел на `string`. Валидация на пустые строки реализована в сервисах/хендлерах.
...
3) Гонка в AddReviewer (проверка количества и вставка)
- Проблема: теоретически две конкурентные вставки могут пройти проверку и обе вставиться.
- Решение: на уровне приложения операции изменения PR выполняются в транзакции через UoW. Для сценариев, требующих строгой гарантии, используется `LockPRByID` и последовательные шаги в рамках одной tx. Дополнительно ограничение `<=2` обеспечивается инвариантами в app-слое.

4) Непоследовательная обработка already-exists в AddMember
...
5) Ошибка `22P02` при старой схеме
- Проблема: ранее в логах были ошибки `invalid input syntax for uuid`.
- Решение: переход на TEXT + ревизия обработки 22P02 (удалена там, где неактуальна).

6) Идемпотентность Merge и UpdateStatus
...
- Решение: добавлены индексы; скорректированы запросы.

8) Поведение `/users/getReview` при несуществующем пользователе
