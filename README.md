...
```bash
make up         # соберёт образы, применит миграции, поднимет сервисы
...
Handlers реализованы как отдельные файлы на метод с Request/Response структурами внутри.
...
8) Логи: избыточные моки
- Проблема: моки логгера усложняли тесты и не приносили пользы.
- Решение: в тестах используем реальный логгер; моки логгера удалены.

9) Валидатор создавался на каждый запрос
- Проблема: накладные расходы на рефлексию.
- Решение: общий инстанс валидатора (утилита в `utils/validator.go`) и вызов `utils.Validate`.

10) Идемпотентность Merge и UpdateStatus
...
## Дальнейшие планы
- Интеграционные/E2E тесты репозиториев и HTTP-слоя
- Консистентные доменные коды ошибок для всех кейсов (расширение `utils.HTTPStatusToCode` при необходимости)
- Доп. ограничения в БД (триггеры/констрейнты) для жёсткого обеспечения инварианта `<=2` ревьюверов
- Нагрузочное тестирование и отчёт

---
Если что-то не запускается или возник вопрос — откройте issue или напишите в комментарии к PR.
